...
impl fmt::Display for TcpFlags {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if self.ns { flags_repr.push("NS"); }
        if self.cwr { flags_repr.push("CWR"); }
        if self.ece { flags_repr.push("ECE"); }
        if self.urg { flags_repr.push("URG"); }
        if self.ack { flags_repr.push("ACK"); }
        if self.psh { flags_repr.push("PSH"); }
        if self.rst { flags_repr.push("RST"); }
        if self.syn { flags_repr.push("SYN"); }
        if self.fin { flags_repr.push("FIN"); }
